...
In this post, I will prove that the unitary, orthogonal, and orthogonal symplectic groups are compact. Compactness of these groups is important in order to talk about integrating over the groups with the Haar measure.
...
$$O(n) = \set{M \in \bbR^{n\times n} \mid M^T M = I}.$$
...
Therefore, $O(n)$ is bounded. Now we must prove that $O(n)$ is closed. To do this, we use Lemma 1 from [this post of mine](compactness-on-topological-spaces-and-why-it-is-important.html), which states that for a continuous function the preimage of a closed set. We can consider the continuous function
...
$$U(n) = \set{M \in \bbC^{n\times n} \mid M^\dagger M = I}.$$
