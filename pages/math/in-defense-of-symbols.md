...
To understand this last step, recall that $$\parentheses{A^a}^b = A^{ab}$$. Thus we now know $$A$$. This illistrate another reason we always use symbols instead of numbers; namely, I had no idea what value $$A$$ was actually going to take. It was just some indeterminant value that we had to figure out.
...
As a neumonic, this is often referred to as the "Pert" equation.
...
We are plotting $$f(t) / P$$, where recall that $$P = f(0)$$, the initial amount of money that you put into the account. From the green line, we can see that with an interest rate of 10% and compounding continuously ($$h = 0$$), after 25 years your $$P$$ dollars will become $$\approx 12P$$ dollars.
...
So $$3! = 3 \times 2 \times 1$$, and etc. The only potentially confusing term is $$0!$$, which is defined to be $$1$$. The second new symbol is the summation symbol, defined by

$$\sum_{i=1}^n f_i = f_1 + f_2 + f_3 + \dots + f_{n-1} + f_n,$$

where each $$f_i$$ is just some number, so that $$(f_1, f_2, \dots, f_n)$$ is a sequence of numbers. The third new symbol that we introduce is the infinity symbol, notated $$\infty$$. Roughly speaking, this means a number that is bigger than all other numbers. To express an infinite sum, then, we can take the limit

$$\lim_{n\to\infty}\sum_{i=1}^n f_i = f_1 + f_2 + f_3 + \dots$$

For brevity, the limit is often removed and it is notated as $$\sum_{i=1}^\infty$$. With these new symbols, we can compactly express $$e$$ as
...
Notice that in this entire post, it was necessary to introduce many symbols, and essentially no numbers were used. We used letters in place numbers so that we can express them compactly, and so that we can adjust their values to compare their effects. We needed to introduce the $$\lim$$ symbol to be able to talk about continuously compounded interest. In order to express $$e$$, we introduced the summation notation $$\sum$$ and the factorial symbol $$!$$. On the right hand side of equation $$(\star)$$, we had the quantity

$$\frac{f(t+h) - f(t)}{h}.$$

This ends up turning up often in math and physics, and so we give it a symbol. Oftentimes it is expressed as $$\Delta f(t)$$ or some variant of that. Even more often, the limit of this expression as $$h \to 0$$ shows up. So we define a symbol for that as well, namely
...
So, long story short, that's why I never use numbers.
