...
**Definitions:**
- $S$ denotes the set of configurations of the state space. For example, for a classical spin 1/2 Ising model on two spins, $S = \curlybrackets{\uparrow \uparrow, \uparrow\downarrow,\downarrow\uparrow,\downarrow\downarrow}.$
...
Thus,
\begin{equation}
  F_\theta^{\text{MF}} = \expval{H}\_{H_\theta'} - \beta \frac{\partial F_\theta'}{\partial \beta}.\label{eq:mf}
\end{equation}
...
The point of all this is that $H_\theta'$ is chosen to be noninteracting, and thus simple. All the expectation values in \eqref{eq:mf} are taken with respect to the noninteracting Hamiltonian $H_\theta'$, and so are doable. Then we simply minimize over $\theta$ to get a noninterating approximation of $F$. How good this approximation is is the subject of the Ginzberg Criterion, which I will hopefully cover in a future post.
...
(here $h$ is taking the role of $\theta$ from above). $h$ acts as an effective field that mimicks the all-to-all interactions from the original Hamiltonian. The partition function of $H_h'$ is simply $Z_h' = 2^n\cosh^n\pargs{\beta h}$. Therefore,
...
$$0 = -J n(n-1)\frac{\tanh\pargs{\beta h}}{\cosh^2\pargs{\beta h}} + \frac{n h}{\cosh^2\pargs{\beta h}}.$$
...
Therefore, the average magnetization predicted by mean field theory has a (second order) phase transition at $\gamma = 1$. The $\bbZ_2$ spin flip symmetry is spontaneously broken. By numerically solving \eqref{eq:mag}, we can generate the following plot of the absolute value of the average magnetization per spin as a function of $\gamma$.

<img style="max-width: 100%" src="media/phasetransition.png" />

By Taylor expanding $\tanh\pargs{\gamma m}$ around $m = 0$, \eqref{eq:mag} becomes

$$m \approx \gamma m - \frac{\gamma^3 m^3}{3}.$$

Thus, around the critical point $\gamma = 1$, the magnetization scales as

$$m \approx \sqrt{3\gamma-3}.$$
